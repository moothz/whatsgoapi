...
import (
	...
	config_env "whatsgo/pkg/config/env"
)
...
func (c *Config) CreateUsersDB() (*gorm.DB, error) {
	log.Printf("[DEBUG] Connecting to database on: %s\n", c.postgresUsersDB)
...
func Load() *Config {
	...
	if postgresUsersDB == "" && (postgresHost == "" || postgresPort == "" || postgresUser == "" || postgresPassword == "" || postgresDB == "") {
		log.Fatalf("[FATAL] [CONFIG] required database configuration variables are missing. Please check your environment configuration.\n")
	}
...
func panicIfEmpty(key, value string) {
	if value == "" {
		if os.Getenv("DEBUG_ENABLED") != "1" {
			log.Printf("[INFO] You are NOT on development mode\n")
		}
		log.Fatalf("[FATAL] [CONFIG] required configuration variable is missing. Please check your environment configuration.\n")
	}
}

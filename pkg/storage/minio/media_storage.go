...
// Format: whatsgo-medias/{filename}
func generateFilePath(fileName string) string {
	return fmt.Sprintf("whatsgo-medias/%s", fileName)
}

// resolveFilePath determines if the input is a full path or just a filename
// If it's just a filename, it assumes it's in the whatsgo-medias folder
...
func (m *MinioMediaStorage) resolveFilePath(ctx context.Context, fileNameOrPath string) (string, error) {
	...
	// If it's just a filename, assume it's in the whatsgo-medias folder
	return fmt.Sprintf("whatsgo-medias/%s", fileNameOrPath), nil
}
...
func NewMinioMediaStorage(
	...
	if !exists {
		err = client.MakeBucket(context.Background(), bucketName, minio.MakeBucketOptions{Region: region})
		if err != nil {
			return nil, fmt.Errorf("failed to create bucket: %w", err)
		}
		fmt.Printf("Successfully created bucket %s on server %s\n", bucketName, endpoint)
	}
